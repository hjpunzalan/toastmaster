...
describe("render user details", () => {
  ...
  test("display user first name", () => {
    ...
    wrapper
      .find(".Dashboard__user-name")
      .props()
      .children.forEach((el) => {
        if (
          el.props &&
          el.props.children === initialProps.currentUser.firstName
        )
          firstNameFound = true;
      });
  ...
  test("display links", () => {
    const updateProfileProps = wrapper
      .find(".Dashboard__links .Dashboard__links-link")
      .get(0).props;
    const changePasswordProps = wrapper
      .find(".Dashboard__links .Dashboard__links-link")
      .get(1).props;
